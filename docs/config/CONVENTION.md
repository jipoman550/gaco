...
⚠️ **모든 커밋 메시지(Subject 및 Body 전체)는 반드시 100% 영어(English)로만 작성해야 합니다. 절대 한국어를 사용하지 마세요.**
...
## ⚠️ 경고
1. **English Only (영문 작성 필수)**: 제목(<subject>)과 본문(<body>)을 포함한 모든 텍스트는 반드시 영어로만 출력하세요. 한국어 설명이나 주석은 절대 금지합니다.
2. **Fact-Only (사실 확인)**: 오직 `git diff`에 녹색(+)으로 추가되거나 빨간색(-)으로 삭제된 코드 조각 및 파일명만 기반으로 작성하세요.
3. **No Speculation (추측 금지)**: `git diff`에 단 한 줄도 언급되지 않은 임의의 기능, 파일 이름, 로직을 본문에 추가하는 즉시 당신의 임무는 실패한 것으로 간주합니다. 솔직하고 직관적으로만 작성하세요.
