...
def initialize_gemini_client(api_key: str) -> genai.GenerativeModel:
    """
    ...
    Returns:
        GenerativeModel: 초기화된 Gemini 모델 인스턴스
...
def generate_commit_message(model: genai.GenerativeModel, system_prompt: str, diff: str) -> str:
    """
    System Prompt와 Diff를 조합하여 Gemini API로 커밋 메시지 생성

    Args:
        model: 초기화된 Gemini 모델
...
아래는 git diff --cached의 결과입니다. 이 변경사항을 분석하여 적절한 커밋 메시지를 생성해주세요.
...
def main() -> int:
    ...
    try:
        ...
        print("\n📌 Step 4: 커밋 메시지 생성 중...")
        commit_message = generate_commit_message(model, system_prompt, diff)
        print("✅ 커밋 메시지 생성 완료")
